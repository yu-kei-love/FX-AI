# ===========================================
# scripts/backtest_v040.py
# v0.40: 節リズム 4特徴量追加版 Stage1 (LGB単体) バックテスト
#
# 2024年テスト、全6券種 x 3パターン
...
def run(is_midnight=False):
    ...
    log(f"\n=== v0.40 {label_ja} backtest ===")

    model_path = MODEL_DIR / f"stage1_{label}_v0.40.pkl"
    ...
    # senshu_name を attach (entries_df は raw データ、sha_ban 列のまま)
    ...
    X = features[FEATURE_NAMES_V040].fillna(0).values
...
def summarize_and_save(results):
    ...
    out_path = REPORT_DIR / "v040_comparison.json"
...
def main():
    log("=" * 60)
    log("v0.40 節リズム特徴量 backtest (2024年)")
    log("=" * 60)
    results = []
